...
public class PhraseRecognizer implements IRecognizer {
	...
	public PhraseRecognizer(HashMap<String, HashMap<String, Integer>> dataSource) {
	    super();
	    setDataSource(dataSource);
	}
  
	public void setDataSource(HashMap<String, HashMap<String, Integer>> dataSource) {
	...
	public List<Token> recognize(List<Token> tokens) {
		...
		TOKEN_LOOP:
			for (int i = 0; i < numTokens; i++) {
		      ...
		      HashMap<String, Integer> leadMap = dataSource.get(word);
		      if (leadMap == null)
		    	  continue;
		      Set<Entry<String, Integer>> phrases = leadMap.entrySet();
		      for (Entry<String, Integer> phraseLength: phrases) {
		    	  String phrase = phraseLength.getKey();
		          int numWords = phraseLength.getValue();
